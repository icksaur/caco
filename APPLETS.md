...
| `files` | Tabbed file viewer — markdown, image, html, diff (unstaged / staged). Ctrl+P opens a fuzzy file picker. Default landing for `?openPath=ABS[&diffMode=staged]` and `?openFinder=1&openFinderRoot=ABS`. |
...
| `git-diff` | `files&openPath=ABS[&diffMode=staged]` (per-file; ref-bearing URLs redirect to `git-status&path=REPO`) |
