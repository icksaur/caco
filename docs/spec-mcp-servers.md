...
`{clientRunning:false, servers:[]}`. **Critical mechanism:** tools come from the
...
- **Discovery needs a running client** (fact): `rpc.tools.list`/`rpc.mcp.list`
  ...
  never an error.
- **MCP tools come from the session-scoped RPC** (invariant): tool discovery must
  use `session.rpc.mcp.listTools({serverName})`, never client-level `tools.list`
  (built-in model tools). Regressing to the client RPC silently returns "no tools".
...
`disabled/not_configured ○`. Tool lists default collapsed; toggling re-fetches (cheap,
keeps state fresh). `configExists` gates an "edit mcp-config.json" link into the text
editor. The auth section is driven by 401s surfaced from MCP calls, not discovery.
...
- Observable: with a running session and ≥1 MCP server, the applet lists each server with a status icon and an expandable tool list; no session → "Start a session to discover servers and tools".
- Gates: typecheck ×2, lint:strict, full tests, build:client.
- Oracles: `buildMcpServerPayload` response shape (per-server tools, null-normalized fields, empty-tools default) — `tests/unit/mcp-server-payload.test.ts`. OAuth PKCE/refresh + store logic — `tests/unit/mcp-auth-routes.test.ts` (duplicated route logic + real `refreshAccessToken`), `mcp-auth-store-atomic.test.ts`, `mcp-discovery.test.ts`; workspace path-security — `tests/unit/mcp-routes.test.ts` (duplicated `isPathAllowed`, not mounted routes). NOT covered by tests: the mounted `/api/mcp/servers` route wiring, the `mcp.listTools` RPC call, and the applet render (by-construction / visual). Same-origin is covered separately by `same-origin-middleware.test.ts`, not the MCP tests.
...
| # | Step | Files | Oracle |
|---|------|-------|--------|
| 1 | `listMcpServers()` (status) + `listMcpTools(serverName)` (per-server tools) via session RPC | `src/session-manager.ts` | by-construction |
| 2 | `GET /api/mcp/servers`: per-server tool fetch + `buildMcpServerPayload` | `src/routes/workspace-api.ts` | `mcp-server-payload.test.ts` |
| 3 | Applet discovery section (status icons, collapsible tools) | `applets/mcp-servers/{content.html,script.js,style.css}` | visual |
