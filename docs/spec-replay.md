...
**Status:** Step 2 (the **primary fix** — abort-before-retry, killing the dual-writer disk contamination that is the dominant cause) is **done**: `retryWithFreshClient` aborts the original generation first and only resends when it confirms the original stopped (`abortStaleGeneration` returns a safe/unsafe boolean) or bails when the dispatch already completed (`isCompleted`) — so a second writer can never contaminate `events.jsonl`. Wired into both retry triggers, covered by `tests/unit/dispatch-retry.test.ts`. Known follow-up (review SHOULD): when abort cannot confirm the original stopped, the retry returns `null` and the caller surfaces a generic "not responding" error — a status enum would let it distinguish "kept the original writer" from a true failure. Steps 3–5 (the render-level epoch fence + reconnect-overlap guard) are **deferred follow-ups**: defense-in-depth for the lower-frequency same-epoch live/replay overlap, needing their own tail-preserving design (per review) before implementing.
