...
- `GET /api/mcp/auth/registry-servers` - List the SDK-metadata server names the tool-key registry still knows (learned keys or stale correlation orphans) — the candidate set for the "forget unknown tools" action. Returns `{ servers: string[] }`.
- `POST /api/mcp/auth/forget-unknown` - Operator-explicit "forget unknown tools" purge: removes all learned keys + correlation for the confirmed-stale server names (the only way to converge a stranded legacy `ADO-*` entry whose server is gone). Body: `{ servers: string[] }` (non-empty; no automatic sweep). Also un-latches the keys from auto-defer and removes them from active sessions' live exclusions so the reminder stops advertising them. Returns `{ removed, persisted, failedSessions: string[] }`; 400 on an empty/invalid list.
