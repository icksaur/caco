...
Add a reliable background trigger for history rotation (`docs/history-rotation-spec.md`) so big sessions get rotated when they're genuinely cold — not only on the incidental `stop()` edges (eviction/delete/changeCwd). Trigger: **once on boot + every 4 hours**.
