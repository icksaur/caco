...
describe('request-metrics-log', () => {
  it('appends a row and reads it back with sessionId, gitSha, and ts', async () => {
    const mod = await import('../../src/request-metrics-log.js');
    mod.appendRequestMetrics('sess-1', ROW, process.cwd());
    ...
    expect(typeof rows[0].gitSha).toBe('string');
    expect(rows[0].gitSha.length).toBeGreaterThan(0);
  ...
  it('accumulates multiple rows as JSONL', async () => {
    const mod = await import('../../src/request-metrics-log.js');
    mod.appendRequestMetrics('sess-1', ROW, process.cwd());
    mod.appendRequestMetrics('sess-2', { ...ROW, requestTurns: 7 }, process.cwd());
    const rows = mod.readRequestMetrics();
  ...
  it('tolerates a corrupt line without throwing', async () => {
    ...
    mod.appendRequestMetrics('sess-2', ROW, process.cwd());
    const rows = mod.readRequestMetrics();
    expect(rows.length).toBe(2);
