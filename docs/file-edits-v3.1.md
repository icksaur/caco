...
- Binary files. Filtered out server-side already.
- Files outside the session's `cwd`.
- Tree view / browser. That's a separate V3 item.
...
│ [Search files: typing...]            │
├──────────────────────────────────────┤
│  src/file-edits-store.ts             │  ← selected
│  src/git-edit-poller.ts              │
│  src/routes/file-edits.ts            │
...
- Render up to 50 results below; scrollable if more.
...
1. **Path is in the current dirty set.** The next poll's `caco.edit`
   will include it as a normal `EditEntry`. We don't need a special
   endpoint — the client persists the path, creates a placeholder,
   and waits ~1.5s for the poll to fill it in. Or, for instant feedback,
   we can issue a manual `POST .../refresh` after picking.
2. **Path is tracked + clean.** Reuse `buildCleanEntry(repoRoot, relPath)`
   — already V2.1 logic. Returns an `EditEntry` with `status: 'clean'`
   and a `fullFile` payload (workLines = headLines, hunks = []).
3. **Path is untracked.** Currently the poller emits an `untracked`
   EditEntry with a synthesized "+all" diff. For the picker we want
   the same shape but without forcing the file into the dirty set.
   Add a small helper: `buildUntrackedEntry(repoRoot, relPath)` reads
   the working-tree file, builds `fullFile` with `headLines: null` and
   `workLines: <file contents>`, status `'untracked'`. (The `untracked`
   status already triggers all-add rendering — V2 buildRows handles
   `headLines: null`.)
...
- `404` — session unknown, or path doesn't exist, or path is outside
  the repo root.
- `400` — relativePath missing, contains `..`, absolute, or otherwise
  unsafe.

Behavior:
1. Resolve `repoRoot` from the session's cwd (using `findRepoRoot`).
2. Validate relativePath: must be a relative path, no `..` segments,
   no leading `/`.
3. `git status --porcelain=v1 -z -u --no-renames -- <relPath>` to find
   the path's status. **Why per-path status:** avoids walking the
   entire repo for the common case; finishes in <20ms even on huge
   repos. Falls back to file-exists check if git returns nothing.
4. Branch:
   - status = `M`/`A`/`D`/`R`/`C` → call existing `buildEntry`.
   - status = `??` (untracked) → call new `buildUntrackedEntry`.
   - no status, file exists in HEAD → call `buildCleanEntry`.
   - no status, file doesn't exist in HEAD or working tree → 404.
...
- `pickerFetchToken: number` (for cancellation — only the latest fetch
  wins)
- `cachedCwd: string` (set on session change for query construction)
...
async function pickFile(relativePath) {
  closePicker();
  // De-dismiss in case the path was previously X'd
  if (dismissed.has(relativePath)) {
    dismissed.delete(relativePath);
  }
  // De-dupe with existing cards
  if (cards.has(relativePath)) return;
  // Materialize entry server-side
  const res = await fetch(
    `/api/sessions/${encodeURIComponent(sessionId)}/file-edits/open`,
    {
      method: 'POST',
  ...
  // applyEdits handles new-card creation + persist scheduling
  applyEdits([edit], [], []);
}
```

`applyEdits` already handles the rest — it sees no existing card,
appends one, calls `schedulePersist()`. Because this is in
`Autoscroll` mode (or `Sticky` if the user was reading) the card just
appears at the bottom; existing scroll rules apply.
